...
description: Rakenna perussovellus, joka analysoi tekstiä käyttämällä Microsoftin kognitiivisten palvelujen tekstianalysoinnin ohjelmointirajapintaa (API).
services: ''
suite: powerapps
...
Kun tekstianalysoinnin API on valmis ja toimii, voit luoda siihen yhteyden PowerAppsista ja rakentaa sovelluksen, joka kutsuu API:a. Tämä on yksittäinen näyttösovellus, joka tarjoaa samantapaiset toiminnot kuin tekstianalysoinnin API-sivulla oleva kokeiluversio. Aloitetaan sovelluksesi rakentaminen!

### <a name="create-the-app-and-add-a-connection"></a>Luo sovellus ja lisää yhteys
Ensin sinun täytyy luoda tyhjä puhelinsovellus ja lisätä yhteys **tekstianalyysiyhteyden** avulla. Jos tarvitset lisätietoja näistä tehtävistä, katso kohdat [Sovelluksen luominen alusta alkaen](get-started-create-from-blank.md) ja [Yhteyksien hallinta Power Appsissa](add-manage-connections.md).

1. Siirry sivustolle [Web.powerapps.com](https://web.powerapps.com), valitse **Aloita tyhjästä** > ![Sovelluksen puhelinkuvake](./media/cognitive-services-api/icon-phone-app.png) (puhelin) > **Tee tämä sovellus**.
...
3. Lisää esittelytekstiä varten **Selite**- ohjausobjektin teksti.
...
7. Lisää kolme **Selite**-ohjausobjektia. Kahdessa ensimmäisessä on kielten ja asenteen API-kutsujen vastaukset; kolmannessa on näytön alareunassa olevan valikoiman esittely.

8. Lisää **Tyhjä pystysuuntainen valikoima** -ohjausobjekti ja lisää valikoimaan **Selite**- ohjausobjekti. Valikoimassa on API-kutsusta saadut avainlauseet. Sovelluksen pitäisi nyt näyttää samalta kuin seuraavassa kuvassa.
   
    ![Sovellus ja sen selitteet ja valikoima](./media/cognitive-services-api/partial-app-step3.png)

9. Valitse vasemmasta ruudusta **Näyttö1** > kolme pistettä (**...** ) > **Poista** (et tarvitse tämä näyttöä sovelluksessa).

Jotta voimme pitää tämän sovelluksen yksinkertaisena, keskitymme tekstianalyysin API:n kutsumiseen. Voisit kuitenkin lisätä sovellukseen muita ominaisuuksia, kuten logiikan, jolla näytetään ja piilotetaan ohjausobjekteja valittujen valintaruutujen mukaan, vianhallinnan tilanteessa, jossa käyttäjä ei valitse mitään vaihtoehtoa, jne.

### <a name="add-logic-to-make-the-right-api-calls"></a>Lisää logiikka, jolla tehdään oikeat API-kutsut
Nyt sinulla on hieno sovellus, jolla ei vielä ole mitään käyttöä. Se ongelma korjataan ihan pian. Ennen kuin perehdymme yksityiskohtiin, on syytä ymmärtää malli, jonka mukaan sovellus toimii:
...
* Lisää kunkin kutsun tulokset asianmukaiseen kokoelmaan:
    ...
    * Lisää **sentimentCollect**-kokoelmaan tekstin sisältämän asenteen **pistemäärä**, joka on 0–1 siten, että 1  on yhtä kuin 100 %:sen positiivinen.
...
1. Määritä kielen selitteen **Teksti**- ominaisuudeksi: `"The language detected is " & First(languageCollect).name`.
...
2. Määritä asenne-selitteen **Teksti**-ominaisuudeksi: `"The sentiment score is " & Round(First(sentimentCollect.Value).Value, 3)\*100 & "% positive."`.
...
3. Määritä avainlauseiden valikoiman **Kohteet**-ominaisuudeksi: `phrasesCollect`.
...
Nyt kun sovellus on valmis, suorita se ja katso, miten se toimii: napsauta tai napauta oikeassa yläkulmassa olevaa Suorita-painiketta. ![Suorita sovellus](./media/cognitive-services-api/icon-run-app.png). Seuraavassa kuvassa kaikki kolme asetusta ovat valittuna ja teksti on sama kuin tekstianalyysin API-sivulla oleva oletusteksti.
...
Toivottavasti tiedät nyt hieman enemmän tekstianalyysin API:sta ja sinulle on ollut ilo nähdä, kuinka se yhdistetään sovellukseen. Kerro meille, jos toivot meidän kertovan tarkemmin joistain muista kognitiivisista palveluista (tai mistä tahansa palveluista). Kuten aina, voit lisätä kommentteihin palautetta tai kysymyksiä.
