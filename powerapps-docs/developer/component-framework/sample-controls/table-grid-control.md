---
title: " Taulukko ruudukon komponentti | Microsoft Docs"
description: Taulukko ruudukon osan toteutus
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340203"
---
# <a name="implementing-table-grid-component"></a>Taulukko ruudukon osan toteutus

Tässä esimerkissä esitellään, miten luodaan yksinkertainen tieto joukko-osa, näkymän sarakkeen metatietojen sidonta, tietueiden sidonta, enemmän tietueita sivutus-ja tietue siirtymästä lomakkeeseen.
Component header-sarakkeet ja sisäiset tietue arvot on sidottu aiemmin luotuihin näkymiin.

> [!div class="mx-imgBorder"]
> ![Taulukko ruudukon osan](../media/table-grid-control.png "taulukko ruudukon osa")
...
Sarake otsikko sidotaan näkymään:

Näytä sarake tiedot ovat `context.parameters.[dataset_property_name].columns`. Se on taulukko tyyppi.

Tietue sidonta:

- Lajitellut tietue tunnukset ovat `context.parameters.[dataset_property_name].sortedRecordIds`
- Kaikki tietueiden tiedot ovat `context.parameters.[dataset_property_name].records` 
- @No__t_0 kullekin tietue objektille 
- Muotoiltu arvo voidaan noutaa `getFormattedValue` 

Lataa lisää tieto sivu tarvittaessa:

`context.parameters.[dataset_property_name].paging` tarjoaa sivutus toimintoja, kuten `hasNextPage` ja `loadNextPage` tietoja. @No__t_0 painike näytetään, jos sillä on seuraavan sivun tiedot.

Tämä malli esittelee myös, miten komponentti kuuntelee säiliön kokoa. 

@No__t_0 menetelmää tulee kutsua [init](../reference/control/init.md) -menetelmässä, jotta `mode.allocatedWidth` ja `mode.allocatedHeight` annetaan aina, kun [updateview](../reference/control/updateview.md) kutsutaan. Jos tätä menetelmää ei kutsuta ensin, sillä ei ole `allocatedWidth` ja `allocatedHeight`.

Jos allocatedHeight-arvo on – 1, korkeutta ei ole rajoitettu. Komponentin on säädettävä sen korkeutta annetun leveyden perusteella.
