---
title: " Taulukon komponentti | Microsoft Docs"
description: Taulukko komponentin toteutus
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340088"
---
# <a name="implementing-table-component"></a>Toteutetaan taulukko komponenttia

Tämä malli komponentti muodostaa taulukon, jossa on kaksi saraketta. Vasemmassa sarakkeessa näytetään ohjelmointi raja pinta menetelmän tai-ominaisuuden nimi, ja oikeanpuoleisessa sarakkeessa näytetään ohjelmointi raja pinnan palauttama arvo. Voit avata tämän osan eri laite tyypille tai muokata kieli-tai käyttäjä asetuksiasi, jotta näet arvot oikein taulukossa.

> [!div class="mx-imgBorder"]
> ![Taulukon komponentin](../media/table-control.png "taulukko-osa")
...
Tässä esimerkissä on esimerkkejä siitä, miten voit käyttää menetelmiä `IClient, IUserSettings, IUtility, IFormatting interfaces`.

Tämä osa esittelee myös kaksi apuohjelma funktiota, `setFullScreen` ja `lookupObjects`. Nämä funktiot käynnistetään napsauttamalla koodi komponentin osana muodostettua painiketta. @No__t_0-painike vaihtaa osaa koko näytön tilassa. @No__t_0-painike avaa valinta ikkunan ja pistää valitun tietueen tekstinä div-muodossa.

Tässä esimerkissä hahmonnetaan HTML-painike ja kiinnitä JavaScript-`onClick` tapahtumakäsittelijä `onLookupObjectsButtonClick` painikkeeseen. Kun napsautat tätä painiketta, käynnistämme `context.utils.lookupObjects()` menetelmän ja välitämme parametrina entiteettien nimien matriisin. 

Tämä menetelmä palauttaa JavaScript Promise-objektin, joka edustaa haku valinta ikkunan kutsun valmistumista tai epäonnistumista. Jos lupaus on ratkennut onnistuneesti, käyttäjän valitsema haku objekti välitetään parametrina takaisinkutsumenetelmään, ja siihen voi viitata muodossa data.id, data.name, data. entityType.

Takaisinkutsumenetelmä tuo nämä tiedot HTML-koodiksi koodi komponentille hahmonnetulle div-kohteelle, joka esittelee valitut tulokset käyttäjälle. Jos lupaus hylätään, virheen takaisinkutsumenetelmä käynnistetään, jos komponentti pystyy käsittelemään virhe skenaarion vastaavasti.
